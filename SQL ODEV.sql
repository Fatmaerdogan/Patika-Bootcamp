...
SELECT IT.ITEMNAME,SUM(AMOUNT)BEBEKOYUNCAKLARISATIS FROM ORDERS O
INNER JOIN ORDERDETAILS OD ON O.ID=OD.ORDERID
INNER JOIN ITEMS IT ON IT.ID=OD.ITEMID
WHERE CATEGORY1='OYUNCAK' AND CATEGORY2='ZEKA GELISTIRICI' AND CATEGORY3='OYUNCAKLAR'
...
--20)--19)[dbo].[PAYMENTS] TABLOSUNDAKÝ SAYISAL DEÐERLERÝ AYRI BÝR KOLON OLARAK YAZIN.
...
SELECT LOWER(NAMESURNAME)KUCUK,* FROM USERS
...
SELECT DATEPART(MONTH,CONVERT(DATE, DATE_))AY FROM [dbo].[orders]
